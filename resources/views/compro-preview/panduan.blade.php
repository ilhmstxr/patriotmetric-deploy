...
<body class="bg-white min-h-screen">
    ...
    <section class="bg-[#1B5E20]">
        <div class="max-w-[900px] mx-auto px-6 md:px-8 py-20 md:py-28 text-center">
            <h1 class="font-['Plus_Jakarta_Sans',sans-serif] font-bold text-[34px] md:text-[50px] text-white leading-tight">
                {{ $heroJudul }}
            </h1>
            @if($heroDeskripsi)
        ...
        <section class="py-16 bg-[#f8fafc]">
            <div class="max-w-[1100px] mx-auto px-6 md:px-8">
                <div class="relative">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-{{ min(count($stepsDaftar), 4) }} gap-8">
                        @foreach($stepsDaftar as $step)
                            <div class="bg-white rounded-3xl border border-[#f1f5f9] shadow-lg p-8 relative z-10">
                                ...
                                <p class="font-['Plus_Jakarta_Sans',sans-serif] font-bold text-[14px] text-[#1b5e20]">
                                    {{ $step['label'] ?? '' }}
                                </p>
                                <h3 class="mt-2 font-['Plus_Jakarta_Sans',sans-serif] font-bold text-[20px] leading-[28px] text-[#1d293d]">
                                    {{ $step['judul'] ?? '' }}
                                </h3>
                                <p class="mt-2 font-['Plus_Jakarta_Sans',sans-serif] font-normal text-[14px] leading-[22.75px] text-[#45556c]">
                                    {{ $step['deskripsi'] ?? '' }}
                                </p>
                            </div>
                        @endforeach
    ...
    {{-- FAQ --}}
    @if(count($faqDaftar) > 0)
        <section class="py-16 md:py-24 bg-white" x-data="{ openFaq: 0 }">
            <div class="max-w-[768px] mx-auto px-8">
                <h2 class="font-['Plus_Jakarta_Sans',sans-serif] font-bold text-[30px] leading-[36px] text-[#1d293d] text-center mb-10">
                    {{ $faqJudul }}
                </h2>
                <div class="flex flex-col gap-4">
                    @foreach($faqDaftar as $i => $faqItem)
                        <div class="bg-white rounded-2xl border border-[#f1f5f9] shadow-sm overflow-hidden">
                            ...
                            >
                                <span class="font-['Plus_Jakarta_Sans',sans-serif] font-semibold text-[18px] leading-[28px] text-[#1d293d] text-left">
                                    {{ $faqItem['pertanyaan'] ?? '' }}
                                </span>
                            ...
                            <div x-show="openFaq === {{ $i }}" x-collapse.duration.300ms>
                                <div class="px-6 pb-5 border-t border-[#f8fafc]">
                                    <p class="pt-3 font-['Plus_Jakarta_Sans',sans-serif] font-normal text-[16px] leading-[26px] text-[#45556c]">
                                        {{ $faqItem['jawaban'] ?? '' }}
                                    </p>
                                </div>
                            </div>
