                            ...
                            <div class="flex items-center gap-3 shrink-0 bg-white border border-[#f1f5f9] rounded-xl px-4 py-3 shadow-sm hover:shadow-md hover:border-[#1B5E20]/20 transition-all duration-300">
                                @if(!empty($inst['logo']))
                                    <div class="size-9 rounded-full overflow-hidden shrink-0">
                                        <img src="{{ '/cms-assets/' . $inst['logo'] }}" alt="{{ $inst['nama'] }}" class="w-full h-full object-cover" />
                                    </div>
                                @else
            ...
            <div class="max-w-[1200px] mx-auto px-6 md:px-8">
                ...
                <div class="grid grid-cols-1 {{ $gridCols }} gap-6 mb-10">
                    @foreach($displayedNews as $i => $item)
                        ...
                        @endphp
                        <a href="{{ route('berita.show', $item->judul) }}" class="group relative block w-full overflow-hidden rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 aspect-[4/3]">
                            @if($newsGambarUrl)
                                <img
                                    src="{{ $newsGambarUrl }}"
                                    alt="{{ $item->judul }}"
