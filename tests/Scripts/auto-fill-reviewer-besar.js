(async () => {
    ...
    console.log(`%c[Start] Menemukan ${questions.length} indikator rubrik. Mulai mengisi skor reviewer secara otomatis (nilai rata-rata besar)...`, "color: #1b5e20; font-weight: bold; font-size: 14px;");

    for (let i = 0; i < questions.length; i++) {
        const q = questions[i];
        
        // 1. Generate skor acak antara 4 dan 5 (nilai besar)
        const randomScore = Math.floor(Math.random() * 2) + 4;
