...
| **GET** | `/api/profile/status` | Mengecek _status verifikasi_ akun dari admin | - | `200 OK` (Data payload status spesifik) |
REQUEST
...
| Method  | Endpoint                  | Fungsi                                                                            | Parameter | Response                            |
| ------- | ------------------------- | --------------------------------------------------------------------------------- | --------- | ----------------------------------- |
| **GET** | `/api/review/submissions` | Menampilkan seluruh _List Final Submit_ (Untuk dikerjakan / disupervisi by Admin) | -         | `200 OK` (Array List _Submissions_) |
...
| **GET** | `/api/review/submissions/{id}` | Mengambil detail spesifik isi klaim/jawaban milik parameter id | `id` (Di URL Param) | `200 OK` (Data spesifik _Submission_ yang akan dinilai) |
REQUEST
...
| **PATCH** | `/api/review/answers/{id}` | _Reviewer_ mencocokan / verifikasi _Verdict_ dan mengubah skor kuesioner spesifik | `id` (Param), `verdict` / angka desimal | `200 OK` (Pesan Update _Verdict_ & kalkulasi re-trigger) |
REQUEST
...
| **GET** | `/api/review/assigned` | _Legacy:_ Mengambil list semua entri yang _Assigned_ ke reviewer ini | `reviewer_id` | `200 OK` (List of Penugasan) |
REQUEST
...
| **POST** | `/api/review/calculate` | _Legacy:_ Men-trigger kalkulasi _manual_ ulang untuk poin yg telah dicek reviewer | `verified_answers` array, `metadata` | `200 OK` (Nilai hasil validasi kalkulasi ulang) |
REQUEST
...
- **Endpoint**: `GET /api/peserta/preview-category/{cat_id}`
...
- **Endpoint**: `GET /api/peserta/results`
...
Contoh Request & Response (Peserta)

GET /api/peserta/questions/1
RESPONSE:
...
GET /api/reviewer/questions/{sub_id}/{cat_id} Mengambil data klaim peserta vs kolom input verifikasi reviewer.
...
GET /api/peserta/steps
GET /api/peserta/{cat_id}
POST /api/peserta/save-progress
GET /api/peserta/preview-category/{cat_id}
POST /api/peserta/finalize
GET /api/peserta/results
