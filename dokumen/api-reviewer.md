...
CHECK
**Kegunaan:** Mengambil data perbandingan (Klaim Peserta vs Input Reviewer) per kategori.
...
Mengambil soal, jawaban peserta (opsi & link), serta data verifikasi reviewer yang mungkin sudah dicicil sebelumnya.
...
- **Side-by-Side Data:** Response harus menyertakan `claim_value` and `evidence_url` dari peserta secara utuh.
...
**Kegunaan:** Tahap akhir oleh Admin untuk merilis nilai ke Peserta.
...
- **Visibility Trigger:** Sebelum status ini `PUBLISHED`, API Peserta (/results) tidak boleh menampilkan skor akhir.

---

# Ringkasan Alur Reviewer

- **Validation of Evidence:** Reviewer bertindak sebagai verifikator atas klaim Peserta. Fokus utama adalah mencocokkan `evidence_url` dengan `claim_value`.
