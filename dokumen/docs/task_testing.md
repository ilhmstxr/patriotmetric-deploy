...
- [ ] **Middleware Check**: Pastikan semua route di `api.php` sudah terbungkus `auth:sanctum` (Terutama group `penugasan/peserta` yang sebelumnya 404).
...
- [ ] **Status IN_PROGRESS**: Verifikasi di database bahwa `peserta@test.com` sudah memiliki record di tabel `Penugasans` dengan status `IN_PROGRESS`.
